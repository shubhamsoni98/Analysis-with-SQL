...
SELECT name FROM Albums ORDER BY release_year ASC LIMIT 1;
...
INSERT INTO Albums (name, release_year, band_id) VALUES ('Your Favorite Album', 2024, LAST_INSERT_ID());

## 7.Delete the Band and Album you added in the previous question:

DELETE FROM Albums WHERE name = 'Your Favorite Album';
DELETE FROM Bands WHERE name = 'Your Favorite Band';

## 8.Get the Average length of all songs:

SELECT AVG(length) AS avg_song_length FROM Songs;

## 9.Select the longest song of each album:

SELECT Albums.name AS album_name, Songs.name AS song_name, MAX(Songs.length) AS longest_song
FROM Albums
JOIN Songs ON Albums.id = Songs.album_id
GROUP BY Albums.id;
...
## 11.Create a decade column with dividing the year:

ALTER TABLE Albums ADD COLUMN decade INT;
UPDATE Albums SET decade = (release_year DIV 10) * 10;

## 12.Filter the Albums which start with the word 'The':

SELECT * FROM Albums WHERE name LIKE 'The%';

## 13.Find the album which released between 2008 and 2013:
